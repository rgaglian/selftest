...
def run_livestatus_exec(device_name, command, arguments, trans, self):
    ...
    for module in device.module:
        ...
        try:
            if module.name == 'tailf-ned-cisco-ios':
                ...
                action_input = device.live_status.generic_ctu_stats__exec.nonconfig_actions.get_input()
                action_input.args = input_args
    ...
    for live_status in device.live_status_protocol:
        try:
            for capability in live_status.capability:
                if capability.uri == "http://tail-f.com/ned/generic-ctu":
                    self.log.info(device_name, ' is a GENERIC-CTU device')
                    self.log.info(arguments, ' Input Arguments')
                    action_input = device.live_status.generic_ctu_stats__exec.nonconfig_actions.get_input()
                    action_input.action.create(arguments)
                    output = action_input = device.live_status.generic_ctu_stats__exec.nonconfig_actions(action_input)
                    return output.result
        except Exception as e:
            self.log.info(device_name, " ERROR: ", str(e))
            error_string = "ERROR: " + str(e)
